...
export async function POST(request: Request) {
    try { // eslint-disable-line no-useless-catch
        const body: Practitioner = await request.json();
        ...
        const created = await fhirStore.create<Practitioner>(body);
...
export async function GET(request: Request) {
    try {
        ...
        const practitioners = await fhirStore.search<Practitioner>('Practitioner', (prac) => {
            if (name) {
                const fullName = prac.name?.map(n => `${n.given?.join(' ')} ${n.family}`).join(' ').toLowerCase();
        ...
        return NextResponse.json({
            ...
            entry: practitioners.map(p => ({ resource: p }))
        });

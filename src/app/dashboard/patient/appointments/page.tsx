import { auth } from "@/auth";
import { getPatientAppointments } from "@/app/actions/clinical";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Calendar, Clock, User as UserIcon, Plus } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import { BookAppointmentForm } from "@/components/fhir-ui/BookAppointmentForm";

export default async function PatientAppointmentsPage() {
  const session = await auth();
  const patientId = session?.user?.id;
  
  const appointments = await getPatientAppointments(patientId || '');

  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">My Appointments</h1>
          <p className="text-muted-foreground">Manage your scheduled visits with doctors.</p>
        </div>
        <Dialog>
            <DialogTrigger asChild>
                <Button className="gap-2">
                    <Plus className="h-4 w-4" />
                    Book New
                </Button>
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Book Appointment</DialogTitle>
                    <DialogDescription>
                        Select a doctor and a suitable time for your visit.
                    </DialogDescription>
                </DialogHeader>
                <BookAppointmentForm patientId={patientId || ''} />
            </DialogContent>
        </Dialog>
      </div>

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {appointments.length === 0 ? (
            <div className="col-span-full text-center py-12 text-muted-foreground bg-muted/20 rounded-xl border border-dashed">
                No appointments found. Book your first one today!
            </div>
        ) : (
            appointments.map((appt: any) => (
            <Card key={appt.id} className="overflow-hidden">
                <div className={`h-2 w-full ${appt.status === 'booked' ? 'bg-blue-500' : appt.status === 'completed' ? 'bg-green-500' : 'bg-gray-300'}`} />
                <CardHeader className="pb-2">
                <div className="flex justify-between items-start">
                    <Badge variant={appt.status === 'booked' ? 'default' : 'secondary'} className="uppercase text-[10px]">
                        {appt.status}
                    </Badge>
                </div>
                <CardTitle className="text-lg flex items-center gap-2 mt-2">
                    <UserIcon className="h-4 w-4 text-primary" />
                    {appt.doctorName}
                </CardTitle>
                <CardDescription>
                    {appt.description}
                </CardDescription>
                </CardHeader>
                <CardContent className="text-sm space-y-2">
                <div className="flex items-center gap-2 text-muted-foreground">
                    <Calendar className="h-4 w-4" />
                    {new Date(appt.start).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    {new Date(appt.start).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
                </div>
                </CardContent>
            </Card>
            ))
        )}
      </div>
    </div>
  );
}
